...
- **Secondary focused tests:** `tests/tracker.spec.ts`, `tests/evaluator.spec.ts`, `tests/git.spec.ts`.
- **Work:** Absorb the review-required redesigns before activation: mutation-free, consistent SQLite read-only snapshots; a contract-generation marker with strict durable registration; rollback and terminal-authority primitives; a semantic provenance builder with manifest reconstruction; empty-manifest and frozen-file fixes; and the shared type, schema, and environment primitives required by those contracts. Keep every primitive unreachable from production start, resume, recovery, migration, classification, and terminal replay.
- **Depends on:** Completed Chunk 03 and its tracker-accounting commit.
- **Exact verification:** `pnpm exec vitest run tests/tracker.spec.ts tests/evaluator.spec.ts tests/git.spec.ts` plus `pnpm run typecheck`. At the Chunk 03R implementation tip, prove zero production activation: the registered tool and controller/recovery routes remain legacy-only, cannot create, resume, classify, migrate, or replay a new-contract run, and cannot durably write or interpret the new contract-generation marker, registration, provenance, or manifest.
- **Implementation commit:** `fix(core): harden activation primitives`
- **Required tracker-accounting commit:** `docs(plan): record handoff chunk 03R`; changes only this checklist and must land before Chunk 04 starts.
- **Rollback:** Revert Chunk 03R and its checklist-only accounting commit before activation. Preserve the completed Chunk 00-03 history and the legacy production route; do not activate partially hardened primitives.
- [ ] Implementation complete
- [ ] Focused verification complete, including zero production activation at the 03R tip
...
| 03R | _unchecked_ | _unchecked_ | _unchecked_ | `docs(plan): record handoff chunk 03R` — _unchecked_ | Before activation, revert Chunk 03R and its accounting commit while preserving completed Chunk 00-03 history and the legacy production route; after activation, roll forward through the hardened shared contract. |
