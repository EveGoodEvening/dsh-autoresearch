...
- **Planning commit:** `docs(plan): scope issues 1 through 3 assessment`.
- **Tracker-only accounting commit:** after review, record the planning SHA, exact changed-path proof, review result, arbitration disposition, rollback, and initial-state evidence. Review the staged tracker-only diff before committing it.
- [ ] Planning diff reviewed and planning commit landed.
